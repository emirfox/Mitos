...
# Let me fill the nan values with zero
data_frame_s_parameters.fillna(0,inplace=True)
...
data_frame_s_parameters = create_table(data_feature,data_frame_s_parameters)
...
data_frame_s_parameters = preprocess_(data_frame_s_parameters)
...
def  check_normality_in_continuous_variables(data_frame_s_parameters):
    ...
    for each in range(0,38):
        ...
        if freq0_n>=0.05:
            x_norm,_ =stats.yeojohnson(data_frame_s_parameters["Frequency {}".format(each)])
            data_frame_s_parameters["Frequency {}".format(each)] = x_norm
...
def svm_classification_model(Attribute_,Label_):
    ...
    #use gridsearch to test all values
    svm_svc_gscv = GridSearchCV(svm_svc_model, param_grid, cv=5)
    #fit model to data
    ...
    # Let me draw the confusion matrix!
    y_predict = cross_val_predict(clf_svm_svc,Attribute_,Label_,cv=5)
    conf_matrix = confusion_matrix(Label_, y_predict)
...
def mlp_classification_model(Attribute_,Label_):
    ...
    #use gridsearch to test all values
    mlp_gscv = GridSearchCV(mlp_model, param_grid, cv=5)
    #fit model to data
    ...
    # Let me draw the confusion matrix!
    y_predict = cross_val_predict(clf_mlp,Attribute_,Label_,cv=5)
    conf_matrix = confusion_matrix(Label_, y_predict)
    ...
    i = 0
    for train, test in cv.split(Attribute_, Label_):
        probas_ = classifier.fit(Attribute_[train], Label_[train]).predict_proba(Attribute_[test])
    ...
    # fig, ax = plt.subplots()
    #
    # for i, (train, test) in enumerate(cv.split(Attribute_, Label_)):
    #     classifier.fit(Attribute_[train], Label_[train])
...
scores_mlp = mlp_classification_model(Attribute_,Label_)
