...
benign_ = data_frame_s_parameters[data_frame_s_parameters["Pathology"]=="benign"]
malignant_ = data_frame_s_parameters[data_frame_s_parameters["Pathology"]=="malignant"]

benign_ = benign_[benign_.columns[:-6]]
malignant_ = malignant_[malignant_.columns[:-6]]




benign_list = []
for each in benign_.columns:
    benign_list = benign_list + benign_[each].to_list()

malignant_list = []
for each in malignant_.columns:
    malignant_list = malignant_list + malignant_[each].to_list()
...
t_test_v = stats.ttest_ind(benign_list,malignant_list)
print(t_test_v)
...
# malignant_list = []
# for each in malignant_.columns:
#     malignant_list = malignant_list + malignant_[malignant_.columns[each]]
#
# print(benign_list)
# print()
...
#
#     Attribute_ = data_frame_s_parameters[data_frame_s_parameters.columns[:-4]]
...
#     y_predict = cross_val_predict(clf,Attribute_,Label_,cv=3)
...
#
#
#     param_grid = {'n_estimators': np.arange(1,60,5),'learning_rate':np.arange(0.1,1,0.05),
...
#
#     #use gridsearch to test all values
#     dtree_gscv = GridSearchCV(abc, param_grid, cv=5,scoring='neg_mean_absolute_error',n_jobs=None,
